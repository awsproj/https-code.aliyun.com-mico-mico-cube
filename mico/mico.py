...
# mico url is subset of hg. mico doesn't support ssh transport
regex_mico_url = r'^(https?)://([\w\-\.]*mico\.(co\.uk|org|com))/(users|teams)/([\w\-]{1,32})/(repos|code)/([\w\-]+)/?$'
# mico sdk builds url
regex_build_url = r'^(https?://([\w\-\.]*mico\.(co\.uk|org|com))/(users|teams)/([\w\-]{1,32})/(repos|code)/([\w\-]+))/builds/?([\w\-]{6,40}|tip)?/?$'

# base url for all mico related repos (used as sort of index)
...
def warning(msg):
    for line in msg.splitlines():
        sys.stderr.write("[mico] WARNING: %s\n" % line)
    sys.stderr.write("---\n")

def error(msg, code=-1):
    for line in msg.splitlines():
        sys.stderr.write("[mico] ERROR: %s\n" % line)
    sys.stderr.write("---\n")
    sys.exit(code)
...
class Hg(object):
    ...
    def isurl(url):
        ...
        if m_url and not re.match(regex_build_url, m_url.group(1)):
            return re.match(regex_hg_url, m_url.group(1)) or re.match(regex_mico_url, m_url.group(1))
        else:
            return False
        '''
        return None
...
class Git(object):
    ...
    def isurl(url):
        m_url = re.match(regex_url_ref, url.strip().replace('\\', '/'))
        if m_url and not re.match(regex_build_url, m_url.group(1)) and not re.match(regex_mico_url, m_url.group(1)):
...
class Repo(object):
    ...
    def fromrepo(cls, path=None):
        ...
        if path is None:
            ...
            if path is None:
                error(
                    "Could not find mico program in current path \"%s\".\n"
                    "You can fix this by calling \"mico new .\" or \"mico config root .\" in the root of your program." % os.getcwd())
        ...
        if repo.scm is None:
            warning(
                ...
                "To fix this you should use \"mico new .\" in the root of your program." % (repo.name, repo.path))
    ...
    def check_repo(self, show_warning=None):
        ...
        if not os.path.isdir(self.path):
            err = (
                ...
                "You can use \"mico deploy\" to import the missing libraries.\n"
                "You can also use \"mico sync\" to synchronize and remove all invalid library references." % (os.path.basename(self.lib), self.path))
        elif not self.isrepo(self.path):
            err = (
                "Library reference \"%s\" points to a folder \"%s\", which is not a valid repository.\n"
                "You can remove the conflicting folder manually and use \"mico deploy\" to import the missing libraries\n"
                "You can also remove library reference \"%s\" and use \"mico sync\" again." % (os.path.basename(self.lib), self.path, self.lib))
...
class Program(object):
    ...
    def __init__(self, path=None, print_warning=False):
        ...
        self.is_classic = os.path.isfile(os.path.join(self.path, 'mico.bld'))

        # is_cwd flag indicates that current dir is assumed to be root, not root repo
        if self.is_cwd and print_warning:
            warning(
                "Could not find mico program in current path \"%s\".\n"
                "You can fix this by calling \"mico new .\" in the root of your program." % self.path)
    ...
    # Gets MiCO OS dir (unified)
    def get_os_dir(self):
        if os.path.isdir(os.path.join(self.path, 'mico-os')):
            return os.path.join(self.path, 'mico-os')
        elif self.name == 'mico-os':
            return self.path
        else:
            return None

    def get_micolib_dir(self):
        if os.path.isdir(os.path.join(self.path, 'mico')):
            return os.path.join(self.path, 'mico')
        else:
            return None

    # Gets mico tools dir (unified)
    def get_tools_dir(self):
        paths = []
        # mico-os dir identified and tools is a sub dir
        mico_os_path = self.get_os_dir()
        if mico_os_path:
            paths.append([mico_os_path, 'tools'])
            paths.append([mico_os_path, 'core', 'tools'])
        # mico-os not identified but tools found under cwd/tools
        paths.append([self.path, 'tools'])
        paths.append([self.path, 'core', 'tools'])
        # mico Classic deployed tools
    ...
    def check_requirements(self, show_warning=False):
        ...
        if missing:
            err = (
                "-----------------------------------------------------------------\n"
                "The mico OS tools in this program require the following Python modules: %s\n"
    ...
    # Routines after cloning mico-os
    def post_action(self):
        mico_tools_path = self.get_tools_dir()

        if not mico_tools_path and (self.is_classic or os.path.exists(os.path.join(self.path, Cfg.file))):
            self.add_tools(os.path.join(self.path, '.temp'))
            mico_tools_path = self.get_tools_dir()

        if not mico_tools_path:
            warning("Cannot find the mico tools directory in \"%s\"" % self.path)
            return False

        if (not os.path.isfile(os.path.join(self.path, 'mico_settings.py')) and
                os.path.isfile(os.path.join(mico_tools_path, 'default_settings.py'))):
            shutil.copy(os.path.join(mico_tools_path, 'default_settings.py'), os.path.join(self.path, 'mico_settings.py'))
    ...
    def add_tools(self, path):
        ...
        with cd(path):
            ...
            if not os.path.exists(tools_dir):
                try:
                    action("Couldn't find build tools in your program. Downloading the mico 2.0 SDK tools...")
                    repo = Repo.fromurl(mico_sdk_tools_url)
                    repo.clone(mico_sdk_tools_url, tools_dir)
                except Exception:
                    if os.path.exists(tools_dir):
                        rmtree_readonly(tools_dir)
                    error("An error occurred while cloning the mico SDK tools from \"%s\"" % mico_sdk_tools_url)
    ...
    def update_tools(self, path):
        ...
        if os.path.exists(os.path.join(path, tools_dir)):
            with cd(os.path.join(path, tools_dir)):
                try:
                    action("Updating the mico 2.0 SDK tools...")
                    repo = Repo.fromrepo()
                    repo.update()
                except Exception:
                    error("An error occurred while update the mico SDK tools from \"%s\"" % mico_sdk_tools_url)

    def get_tools(self):
        mico_tools_path = self.get_tools_dir()
        if not mico_tools_path:
            error('The mico tools were not found in "%s". \nRun `mico deploy` to install dependencies and tools. ' % self.path, -1)
        return mico_tools_path
    ...
    def get_env(self):
        ...
        for c in compilers:
            if self.get_cfg(c+'_PATH'):
                env['MICO_'+c+'_PATH'] = self.get_cfg(c+'_PATH')
    ...
    def get_target(self, target=None):
        ...
        if target and (target.lower() == 'detect' or target.lower() == 'auto'):
            ...
            if targets == False:
                error("The target detection requires that the 'mico-ls' python module is installed.\nYou can install mico-ls by running 'pip install mico-ls'.")
            elif len(targets) > 1:
        ...
        if target is None:
            error("Please specify target using the -m switch or set default target using command 'mico target'", 1)
        return target
    ...
    def get_toolchain(self, toolchain=None):
        ...
        if tchain is None:
            error("Please specify toolchain using the -t switch or set default toolchain using command 'mico toolchain'", 1)
        return tchain
    ...
    def ignore_build_dir(self):
        ...
        if not os.path.exists(build_path):
            os.mkdir(build_path)
        if not os.path.exists(os.path.join(build_path, '.micoignore')):
            try:
                with open(os.path.join(build_path, '.micoignore'), 'w') as f:
                    f.write('*\n')
            except IOError:
                error("Unable to write build ignore file in \"%s\"" % os.path.join(build_path, '.micoignore'), 1)
    ...
    def get_detected_targets(self):
        ...
        try:
            ...
            micos = mico_lstools.create()
            detect_muts_list = micos.list_micos()
            if os.name == 'nt':
...
class Global(object):
    def __init__(self):
        self.path = os.path.join(os.path.expanduser("~"), '.mico')
...
def update(rev=None, clean=False, clean_files=False, clean_deps=False, ignore=False, top=True, depth=None, protocol=None):
    ...
    if top and not rev and repo.isdetached():
        error(
            ...
            "You can checkout a branch using \"%s checkout <branch_name>\" command before running \"mico update\"." % (cwd_type, repo.scm.name), 1)
...
@subcommand('compile',
    ...
    dict(name=['--profile'], action='append', help='Path of a build profile configuration file. Example: mico-os/tools/profiles/debug.json'),
    ...
    dict(name='--app-config', dest="app_config", help="Path of an app configuration file (Default is to look for 'mico_app.json')"),
    help='Compile code using the mico build tools',
    description=("Compile this program using the mico build tools."))
...
def test_(toolchain=None, target=None, compile_list=False, run_list=False, compile_only=False, run_only=False, tests_by_name=None, source=False, profile=False, build=False, clean=False, test_spec=None, app_config=None):
    ...
    with cd(program.path):
        ...
        if run_list:
            popen(['micogt', '--test-spec', test_spec, '--list']
                  + (['-n', tests_by_name] if tests_by_name else [])
                  + (['-V'] if verbose else [])
                  + args,
                  env=env)
        ...
        if run_only or build_and_run_tests:
            popen(['micogt', '--test-spec', test_spec]
                  + (['-n', tests_by_name] if tests_by_name else [])
                  + (['-V'] if verbose else [])
                  + args,
                  env=env)
...
def export(ide=None, target=None, source=False, clean=False, supported=False):
    ...
    # Remember the original path. this is needed for compiling only the libraries and tests for the current folder.
    orig_path = os.getcwd()
    # Change directories to the program root to use mico OS tools
    with cd(program.path):
        tools_dir = program.get_tools()
...
# Test command
@subcommand('detect',
    help='Detect connected mico targets/boards\n\n',
    description=(
        "Detects mico targets/boards connected to this system and shows supported\n"
        "toolchain matrix."))
def detect():
    # Gather remaining arguments
    args = remainder
    ...
    if tools_dir:
        ...
        warning("The mico tools were not found in \"%s\". \nLimited information will be shown about connected mico targets/boards" % program.path)
...
def config_(var=None, value=None, global_cfg=False, unset=False, list_config=False):
    ...
    if list_config:
        ...
        if not p.is_cwd:
            ...
            log("Couldn't find valid mico program in %s\n" % p.path)
        ...
        if global_cfg:
            ...
            if program.is_cwd and not var == 'ROOT':
                error(
                    "Could not find mico program in current path \"%s\".\n"
                    "Change the current directory to a valid mico program, set the current directory as an mico program with 'mico config root .', or use the '--global' option to set global configuration." % program.path)
            with cd(program.path):
                if unset:
...
@subcommand('target',
    ...
    description=(
        ...
        "This is an alias to 'mico config [--global] target [name]'\n"))
...
@subcommand('toolchain',
    ...
    description=(
        "Set or get default toolchain\n"
        "This is an alias to 'mico config [--global] toolchain [name]'\n"))
